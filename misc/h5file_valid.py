...
motgt = pd.read_csv("keypoint_test_resize.txt", sep=" ", index_col = False)
#motgt = motgt.loc[:,'1':'41']
motgt = motgt.drop(motgt.columns[[0]],axis=1)
final = motgt.as_matrix()
#np.savetxt('part_valid.txt', final, delimiter=',',fmt='%.0f')
print(np.shape(final))
final=np.reshape(final,(241520,2))
#final=final[0:2958]
np.savetxt('part_valid_resize.txt', final, delimiter=',',fmt='%.0f')
DATASET1='istrain'
...
fs4=h5py.File('imgname_test17.h5', 'r')
fs5=h5py.File('part_test20.h5', 'r')
