...
function drawSkeleton(input, hms, coords)
    ...
    local partNames = {'lfwheel', 'lbwheel', 'rfwheel',
         'rbwheel', 'rfoglamp', 'lfoglamp',
         'rheadlight', 'lheadlight', 'frlogo', 'frlicense',
         'lremirror', 'rremirror', 'rfrtopcorner','lfrtopcorner',
         'lbcktopcorner', 'rbcktopcorner', 'lrearlamp','rrearlamp',
         'relogo', 'relicnese'}


    local tags = {'index','center','istrain','imgname','person','part'}
    local partColor = {1,1,1,2,2,2,0,0,0,0,3,3,3,4,4,4,5,5,5,5}
...
function drawOutput2(input, hms, coords)
...
function drawOutput(input, hms, coords)
    ...
    im = drawSkeleton(img, preds[tmpIdx]:narrow(2,1,2):clone(), preds[tmpIdx]:narrow(2,5,1):clone():view(-1))
